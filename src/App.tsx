...
export default function App(){
 ...
 {tab==='CAD'&&<section className="cad-grid"><aside className="cad-panel calls"><div className="panel-title"><span>911 CALL QUEUE</span><b>{active.length}</b></div><div className="filter-row"><button className={filter==='ALL'?'active':''} onClick={()=>setFilter('ALL')}>ALL</button>{(['POLICE','FIRE','EMS'] as Service[]).map(s=><button key={s} className={filter===s?'active':''} onClick={()=>setFilter(s)}>{s}</button>)}</div><div className="call-list">{active.filter(i=>filter==='ALL'||i.requiredServices.includes(filter)).map(i=><button key={i.id} className={selected===i.id?'call selected':'call'} onClick={()=>{if(!buildMode)setSelected(i.id)}}><strong>{i.id}</strong><span>{i.status==='NEW'?'☎ INCOMING 911':getIncidentStatusLabel(i.status)} · P{i.priority}</span><small>{i.address}</small><em>{i.summary}</em><label>{i.type} · {i.requiredServices.join(' + ')}</label></button>)}</div>{incident?.status==='NEW'&&<button className="dispatch-new" onClick={accept}>☎ NOTRUF ANNEHMEN</button>}{incident&&incident.status!=='NEW'&&<div className="call-processed">✓ NOTRUF BEARBEITET</div>}{incoming.map((a:any)=><div className="aid-request" key={a.id}><b>CAD REQUEST · {a.fromName}</b><button onClick={()=>answerAid(a.id,true)}>ACCEPT</button><button onClick={()=>answerAid(a.id,false)}>DECLINE</button></div>)}</aside><div className="map-stage"><RealMapView state={game} zoom={zoom} onZoomChange={setZoom} onIncidentSelect={setSelected} buildMode={buildMode} onMapBuild={place}/>{buildMode&&<div className="build-confirm-bar"><b>{meta[buildMode].station.toUpperCase()} · FREIE STANDORTWAHL</b><span>Linksklick auf der Karte baut exakt an diesem Punkt.</span><button disabled={buildPending} onClick={cancelBuild}>{buildPending?'WARTE AUF SERVER':'BAU ABBRECHEN'}</button></div>}<div className="map-banner"><b>{buildMode?`BUILD MODE · ${meta[buildMode].station.toUpperCase()}`:'AUTONOMOUS CAD'}</b><span>{rank} · LEVEL {profile.level}</span><span>{game.world.traffic}% TRAFFIC</span></div></div><aside className="cad-panel units"><div className="panel-title"><span>OWN UNITS</span><b>{myUnits.length}</b></div>{myUnits.map(u=><button key={u.id} className="unit-row" onClick={()=>playRadioPing}><span className={`unit-dot ${u.service.toLowerCase()}`}/><div><b>{u.callsign}</b><small>{u.type} · {getUnitStatusLabel(u.status)}</small></div><em>{u.speed.toFixed(1)} u/s</em>{incident&&u.status==='AVAILABLE'&&<i onClick={e=>{e.stopPropagation();dispatch(u)}}>SEND</i>}</button>)}{!myUnits.length&&<div className="empty-units">Noch keine eigenen Einheiten. Baue deine erste Wache.</div>}{incident&&myUnits.some(u=>u.status==='AVAILABLE')&&<button className="auto-dispatch" onClick={auto}>AUTO DISPATCH</button>}{incident&&incident.status==='RESPONDING'&&<button className="complete-button" onClick={complete}>EINSATZ ABSCHLIESSEN</button>}</aside></section>}
 {tab==='AGENCY'&&<section className="agency-page"><div className="agency-hero"><div><small>AGENCY BUILDER</small><h2>Baue deine eigene Behörde</h2><p>Starte mit einer Wache. Jede Station bringt Fahrzeuge, Kapazität und neue Karriereoptionen.</p></div><div className="agency-budget">{cash(profile.money)}</div></div><div className="service-picker">{(['POLICE','FIRE','EMS'] as Service[]).map(s=><button key={s} className={`service-card ${currentService===s?'selected':''}`} onClick={()=>chooseService(s)} style={{borderTopColor:meta[s].color}}><b>{meta[s].name}</b><span>{meta[s].station}</span><small>LEVEL 1 · AB {cash(meta[s].cost)}</small></button>)}</div><div className="build-grid"><div className="build-card"><div><small>FOUNDATION</small><h3>{meta[currentService].station} 1</h3><p>Freier Standort · eigene Fahrzeuge · eigener Bestand</p></div><strong>{cash(meta[currentService].cost)}</strong><button disabled={buildPending} onClick={()=>startBuild(currentService)}>{buildPending?'BAU WIRD GEPRÜFT':'BAUEN'}</button></div><div className="facility-list">{ownedFacilities.map(f=><div className="facility-manage" key={f.id}><div><b>{f.name}</b><span>LEVEL {f.level} · {f.type}</span></div><button onClick={()=>upgrade(f,'FLEET')}>FLEET +</button><button onClick={()=>upgrade(f,f.type==='POLICE_STATION'?'CELLS':f.type==='FIRE_STATION'?'APPARATUS':'MEDICAL')}>AUSBAU</button></div>)}{!ownedFacilities.length&&<p>Noch keine eigene Wache gebaut.</p>}</div></div></section>}
 {tab==='RANKS'&&<section className="rank-page"><div className="rank-head"><small>CAREER LADDER</small><h2>{meta[currentService].name}</h2><p>{rank} · LEVEL {profile.level}</p></div><div className="rank-grid">{ranks[currentService].map((r,i)=><button key={r} className={i===Math.max(0,profile.level-1)?'rank-card current':i<profile.level-1?'rank-card unlocked':'rank-card'} onClick={()=>setSelectedRank(i)}><span>{String(i+1).padStart(2,'0')}</span><b>{r}</b><small>{i<profile.level?'FREIGESCHALTET':'LOCKED'}</small></button>)}</div><div className="rank-detail"><h3>{ranks[currentService][selectedRank]}</h3><p>Status: {selectedRank<profile.level?'Freigeschaltet':'Noch nicht freigeschaltet'}</p><p>Freischaltung: {rankRewards[currentService][Math.min(selectedRank,rankRewards[currentService].length-1)]}</p><p>Karrierefortschritt basiert auf abgeschlossenen Einsätzen, Reputation und wachsender Behördenkapazität.</p></div></section>}
 {tab==='WORLD'&&<section className="world-page"><div><small>MULTIPLAYER</small><h2>REDWOOD METRO NETWORK</h2><p>{otherPlayers.length+1} Leitstellen verbunden · gemeinsamer Ereignisraum · Mutual Aid</p></div><div className="player-list"><div className="aid-card"><b>DEINE LEITSTELLE</b><span>{profile.playerName} · {meta[currentService].name}</span><small>{myUnits.length} Einheiten · {ownedFacilities.length} Wachen</small></div>{otherPlayers.map((p:any)=><div className="aid-card" key={p.id}><b>{p.name}</b><span>CONNECTED</span><small>Mutual Aid verfügbar</small></div>)}</div>{incident&&otherPlayers.length>0&&<button className="request-aid" onClick={requestAid}>MUTUAL AID ANFORDERN</button>}{notice&&<div className="toast">{notice}</div>}</section>}
 {tutorial&&<Tutorial close={finishTutorial}/>} {notice&&tab!=='WORLD'&&<div className="toast">{notice}</div>}
 </main>
}
