...
const serverFixture=`const c:Client={id:\`player-\${randomUUID()}\`,name:'Dispatcher',ready:false,accountToken:'',remoteAddress};function build(){state.finance.upgrades+=cost;addVehicle(c,service,safePosition);}if(existing){c.accountToken=existing.token;c.name=existing.name}else{c.name=cleanName(m.name);const account=accountFor(c);account.name=c.name}const account=accountFor(c);if(typeof m.name==='string'&&!existing)account.name=c.name=cleanName(m.name);account.lastSeen=Date.now();c.ready=m.ready===true;c.name=initial.name;send(socket,{type:'HELLO_ACK',playerId:c.id`;
...
describe('server runtime policy',()=>{
  ...
  it('stabilizes account ownership across reconnects',()=>{
    ...
    expect(patched).toContain('c.id=`player-${initial.token}`');
