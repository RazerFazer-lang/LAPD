...
describe('American Dispatch domain',()=>{
  ...
  it('localizes every operational status',()=>{
    expect(getUnitStatusLabel('EN_ROUTE')).toBe('AUF ANFAHRT');
    expect(getUnitStatusLabel('RETURNING')).toBe('RÜCKKEHR');
    expect(getUnitStatusLabel('MAINTENANCE')).toBe('WARTUNG');
    expect(getIncidentStatusLabel('NEW')).toBe('NEU');
  });
